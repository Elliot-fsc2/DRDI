    ...
    <section class="space-y-6">
        ...
        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            @forelse ($instructors ?? [] as $instructor)
                {{-- Render the Livewire card component for each Instructor model --}}
                <a wire:navigate wire:key="instructor-{{ $instructor->id }}"
                    class="block h-full transition-transform duration-200 hover:scale-105""
                    href="{{ route('filament.instructor.pages.manage-instructors.{instructor}', ['instructor' => $instructor]) }}">
                    <livewire:instructor-cards :instructor="$instructor" :key="'instructor-card-' . $instructor->id" />
                </a>
            @empty
                <div class="col-span-full rounded-lg bg-white p-12 shadow-sm dark:bg-gray-800">
                    <div class="text-center">
                        ...
                        <h3 class="mt-2 text-sm font-semibold text-gray-900 dark:text-gray-100">No instructors found
                        </h3>
