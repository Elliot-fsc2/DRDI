    ...
    <div class="flex h-full w-full flex-col overflow-hidden rounded-lg bg-white shadow-md dark:bg-gray-800">
        ...
        <div class="flex flex-1 flex-col p-6">
            ...
            <div class="mt-auto flex items-center justify-between">
                @php
                    $currentPersonnel = $group->personnels->where('instructor_id', auth()->user()->instructor->id)->first();
                @endphp
                @if($currentPersonnel)
                <div class="flex items-center">
                    ...
                    <div class="ml-3 text-sm">
                        <p class="font-medium text-gray-700 dark:text-gray-200">
                            {{ $currentPersonnel->instructor->full_name }}
                        </p>
                        <p class="text-xs text-gray-500 dark:text-gray-400">{{ ucwords(str_replace('_', ' ', $currentPersonnel->role)) }}</p>
                    </div>
                </div>
                @endif
