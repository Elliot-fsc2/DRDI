        ...
        @if ($this->proposals->count() > 0)
            <div class="flex flex-col gap-4">
                @foreach ($this->proposals as $proposal)
                    <div wire:click="mountAction('viewProposal', { 'proposal': {{ $proposal }} })"
                        wire:key="proposal-{{ $proposal->id }}"
                        class="group cursor-pointer rounded-lg border border-gray-200 bg-white p-4 transition-all duration-200 hover:border-gray-300 hover:shadow-md dark:border-gray-700 dark:bg-gray-800 dark:hover:border-gray-600">
                        <div class="flex flex-col gap-3">
                            <div class="flex-1">
                                <div class="mb-2 flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-3">
                                    <h3
                                        class="text-base font-medium leading-tight text-gray-900 transition-colors sm:text-lg dark:text-white">
                                        {{ Str::limit(strip_tags($proposal->title), 50) }}
                                    </h3>
                                ...
                                <p class="mb-3 line-clamp-2 text-sm text-gray-600 dark:text-gray-400">
                                    {{ Str::limit(strip_tags($proposal->description), 100) }}
                                </p>
